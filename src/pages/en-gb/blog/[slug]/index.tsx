...
export const getStaticPaths = async () => {
  try {
    ...
    return {
      paths: filteredPost.map((row) => `/en-gb/blog/${row.slug}`),
      fallback: true,
    };
  } catch (error) {
...
const EnGbDetailPage: NextPageWithLayout = () => {
  ...
  const meta = {
    ...
    type: post.type[0],
    url: `${CONFIG.link}/en-gb/blog/${post.slug}`,
  };
  return (
    <>
      <MetaConfig {...meta} />
...
EnGbDetailPage.getLayout = (page) => <>{page}</>;
export default EnGbDetailPage;
