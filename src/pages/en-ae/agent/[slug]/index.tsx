...
import { uaePatientlyAIFaqs } from "src/data/localeFaqs";
...
const UAEAgentDetailPage = ({ data, slug }: InferGetStaticPropsType<typeof getStaticProps>) => {
  ...
  const agentNameUAE = `${agentName} Dubai | AI Caller for UAE Leads`;
  const agentDescription = transformedHero?.description || transformedHero?.subtitle || "Custom AI agent solution for your business needs in UAE";
  const agentImage = transformedHero?.image || "/og.jpg";
  const agentUrl = `https://www.tryagentikai.com/uae/agent/${slug}`;

  // Use locale-specific FAQs for PatientlyAI
  const isPatientlyAI = slug === PATIENTLYAI_SLUG;
  const localeFaqs = isPatientlyAI ? uaePatientlyAIFaqs : null;
...
export default UAEAgentDetailPage;
