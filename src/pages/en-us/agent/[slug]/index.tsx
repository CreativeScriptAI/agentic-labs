...
import { northAmericaPatientlyAIFaqs } from "src/data/localeFaqs";
...
const USAAgentDetailPage = ({ data, slug }: InferGetStaticPropsType<typeof getStaticProps>) => {
  ...
  const agentNameUSA = `${agentName} USA | AI Caller for American Leads`;
  const agentDescription = transformedHero?.description || transformedHero?.subtitle || "Custom AI agent solution for your business needs in the USA";
  const agentImage = transformedHero?.image || "/og.jpg";
  const agentUrl = `https://www.tryagentikai.com/usa/agent/${slug}`;

  // Use locale-specific FAQs for PatientlyAI
  const isPatientlyAI = slug === PATIENTLYAI_SLUG;
  const localeFaqs = isPatientlyAI ? northAmericaPatientlyAIFaqs : null;
...
export default USAAgentDetailPage;
